...
class root_table(table):
    ...
    def unique_tablename(self, node_id, node_name, path, etype='loop'):
        ...
        if etype == 'loop' or len(parents) <= 2:
            table_name = ('%s_%s_%s' % (self.prefix, node_id,
                                        clean_name))[:120]
        else:
            table_name = ('%s_%s_%s_%s' % (self.prefix,
                                           parents[-2], node_id, clean_name))[:120]
            del parents[-1]
            del parents[-1]
        i = 1
        ...
        if node.is_loop():
            ...
            if node.get_max_repeat() != 1:  # Create new table
                ...
                if cur_table.is_match_parent(parent):
                    ...
                    cur_table.sub_tables.append(table(node.id,
                        node.name, path, cur_table))
                ...
                cur_table.sub_tables.append(table(
                    node.id, node.name, path, cur_table))
                cur_table = cur_table.sub_tables[-1]
            ...
            if node.children[0].is_element() \
                ...
                cur_table.field_prefix = '%s' % (
                    node.children[0].valid_codes[0])
            # Find parent loop
            #pdb.set_trace()
            ...
            if node.get_max_repeat() != 1:  # Create sub table
                ...
                cur_table.sub_tables.append(table(node.id,
                    node.name, path, cur_table, 'segment'))
                cur_table = cur_table.sub_tables[-1]
            ...
            vtype += ' NULL'
            vtype += '  -- %s(%s, %s)' % (
                node.data_type, node.min_len, node.max_len)
...
def main():
    ...
    for map_filename in args:
        try:
            ...
            sql = gen_sql(pyx12.map_if.map_if(
                os.path.join(map_path, map_file), param), prefix)
            print sql.generate()
            #print sql
        except IOError:
