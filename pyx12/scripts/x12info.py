...
def main():
    ...
    parser.add_argument(
        "--inplace", "-i", action="store_true", help="Make changes to files in place"
    )
    parser.add_argument(
        "--fixcounting", "-f", action="store_true", help="Try to fix counting errors"
    )
    parser.add_argument(
        "--output-dir",
        "-t",
    ...
    for fn in args.input_files:
        for src_filename in glob.iglob(fn):
            try:
                ...
                (result, headers, node_summary) = get_x12file_metadata(
                    param, src_filename, args.map_path
                )
                if not result:
