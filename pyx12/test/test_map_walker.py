...
class Explicit_Loops(unittest.TestCase):
    ...
    def test_GS_to_ST_277(self):
        ...
        (node, pop, push, walk_errors) = walker.walk_errors(node, seg_data, 5, 4, None)
...
class Implicit_Loops(unittest.TestCase):
    ...
    def test_837i_2420a(self):
        ...
        cmap = pyx12.map_if.load_map_file("837Q3.I.5010.X223.A1.xml", self.param)
        errh = pyx12.error_handler.errh_null()
    ...
    def test_999_2110_IK4(self):
        ...
        errh.reset()
        (node, pop, push, walk_errors) = walker.walk_errors(
            node, seg_data, seg_count=8, cur_line=7, ls_id=None
        )
        apply_walk_errors(errh, walk_errors)
        self.assertNotEqual(node, None, "walker failed to find %s" % (seg_data))
        self.assertEqual(seg_data.get_seg_id(), node.id)
...
class SegmentWalk278(unittest.TestCase):
    ...
    def test_match_regular_segment(self):
        ...
        node = self.map.getnodebypath(
            "/ISA_LOOP/GS_LOOP/ST_LOOP/DETAIL/2000A/2000B/2000C/2000E/2000F/DTP"
        )
        self.assertNotEqual(node, None)
        self.assertEqual(node.base_name, "segment")
...
class LoopPathPopPush837(unittest.TestCase):
    ...
    def test_path_sub_loop(self):
        node = self.map.getnodebypath("/ISA_LOOP/GS_LOOP/ST_LOOP/DETAIL/2000A/2000B/2300/HI")
        seg_data = pyx12.segment.Segment("NM1*82*2*Provider Name*****XX*24423423~", "~", "*", ":")
        (node, pop, push, walk_errors) = self.walker.walk_errors(node, seg_data, 5, 4, None)
        apply_walk_errors(self.errh, walk_errors)
        self.assertNotEqual(node, None)
        self.assertEqual(seg_data.get_seg_id(), node.id)
    ...
    def test_path_up3(self):
        node = self.map.getnodebypath(
            "/ISA_LOOP/GS_LOOP/ST_LOOP/DETAIL/2000A/2000B/2300/2400/2430/SVD"
        )
...
class LoopPathPopPush834(unittest.TestCase):
    ...
    def test_path_repeat(self):
        node = self.map.getnodebypath("/ISA_LOOP/GS_LOOP/ST_LOOP/DETAIL/2000/DTP")
        seg_data = pyx12.segment.Segment("INS*Y*18*030*20*A", "~", "*", ":")
        (node, pop, push, walk_errors) = self.walker.walk_errors(node, seg_data, 5, 4, None)
        apply_walk_errors(self.errh, walk_errors)
        self.assertNotEqual(node, None)
    ...
    def test_path_up(self):
        ...
        self.assertNotEqual(node, None)
        self.assertEqual(seg_data.get_seg_id(), node.id)
        self.assertEqual(get_id_list(pop), ["2100A", "2000", "DETAIL", "ST_LOOP"])
        self.assertEqual(get_id_list(push), ["ST_LOOP"])

    def test_path_up2(self):
        node = self.map.getnodebypath("/ISA_LOOP/GS_LOOP/ST_LOOP/DETAIL/2000/INS")
        seg_data = pyx12.segment.Segment(
            "GS*BE*AAA*BBB*20081116*2044*328190001*X*004010X095A1", "~", "*", ":"
        )
        (node, pop, push, walk_errors) = self.walker.walk_errors(node, seg_data, 5, 4, None)
        apply_walk_errors(self.errh, walk_errors)
        self.assertNotEqual(node, None)
